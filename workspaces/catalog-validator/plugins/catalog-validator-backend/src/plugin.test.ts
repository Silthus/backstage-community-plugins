...
import { catalogValidatorPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [catalogValidatorPlugin],
    });

    await request(server).get('/api/catalog-validator/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/catalog-validator/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/catalog-validator/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/catalog-validator/todos/${createdTodoItem.id}`)
